...
TaiwanStockNews = Load.FinData(
    dataset="TaiwanStockNews", date=date, select="2317"
)
print(TaiwanStockNews[:5])

print(
    "load TaiwanStockPrice {} ".format(TaiwanStockInfo.loc[_index, "stock_id"])
)
TaiwanStockPrice = Load.FinData(
    dataset="TaiwanStockPrice", select="2330", date=date2
)
print(TaiwanStockPrice[:5])

print(
    "load StockDividendResult {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
StockDividendResult = Load.FinData(
    dataset="StockDividendResult", select="2330", date="2010-01-01"
)
print(StockDividendResult[:5])

print(
    "load 財報 FinancialStatements {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
print(
    "load 台股配息 TaiwanStockStockDividend {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
data = Load.FinData(dataset="StockDividend", select="0050", date="2015-01-01")
# data['date'] = data['date'] + '-' + data['period']
# data = data.drop('period',axis = 1)
data = Load.transpose(data)

print(
    "load 借卷融資 TaiwanStockMarginPurchaseShortSale {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
TotalMarginPurchaseShortSale = Load.FinData(
    dataset="TotalMarginPurchaseShortSale", date=date3
)
print(TotalMarginPurchaseShortSale[:5])

print(
    "load 外資買賣 InstitutionalInvestorsBuySell {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
print(
    "load 外資持股 Shareholding {} ".format(TaiwanStockInfo.loc[_index, "stock_id"])
)
...
print(
    "load 資產負債表 BalanceSheet {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
BalanceSheet = Load.FinData(dataset="BalanceSheet", select="2317", date=date3)
print(BalanceSheet[:5])
data = Load.transpose(BalanceSheet)

print(
    "load 股權分散表 TaiwanStockHoldingSharesPer {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
print(
    "load 月營收 TaiwanStockMonthRevenue {} ".format(
        TaiwanStockInfo.loc[_index, "stock_id"]
    )
)
...
USStockPrice = Load.FinData(
    dataset="USStockPrice", select=USStockInfo.loc[20, "stock_id"], date=date
)
...
UKStockPrice = Load.FinData(
    dataset="UKStockPrice", select=UKStockInfo.loc[200, "stock_id"], date=date
)
...
ExchangeRate = Load.FinData(
    dataset="ExchangeRate", select=ExchangeRate_list[3], date=date
)
...
InstitutionalInvestors = Load.FinData(
    dataset="InstitutionalInvestors", date=date
)
...
InterestRate = Load.FinData(
    dataset="InterestRate", select=InterestRate_list[5], date=date2
)
...
CrudeOilPrices = Load.FinData(
    dataset="CrudeOilPrices", select=CrudeOilPrices_list[1], date=date
)
...
CurrencyCirculation = Load.FinData(
    dataset="CurrencyCirculation", select=CurrencyCirculation_list[1], date=date
)
